...
int8 stall_vending_setup(map_session_data* sd, const char* message, const int16 xPos, const int16 yPos, uint8* data, int count)
{
	...
	if (map_getcell(sd->m, xPos, yPos, CELL_CHKNOVENDING)) {
		clif_displaymessage(sd->fd, msg_txt(sd, 276)); // "You can't open a shop on this cell"
		return 1;
	}

	if (map_getcell(sd->m, xPos, yPos, CELL_CHKNOPASS)) {
		clif_displaymessage(sd->fd, msg_txt(sd, 276)); // "You can't open a shop on this cell"
		return 1;
	}

	if (map_count_oncell(sd->m, xPos, yPos, BL_CHAR | BL_NPC | BL_STALL, 1)) {
		clif_displaymessage(sd->fd, msg_txt(sd, 276)); // "You can't open a shop on this cell"
		return 1;
	}
	...
	st->id = st->vender_id;
	st->type = BL_STALL;
	st->m = map_mapindex2mapid(sd->mapindex);
	st->x = xPos;
	st->y = yPos;
...
int8 stall_buying_setup(map_session_data* sd, const char* message, const int16 xPos, const int16 yPos, const struct STALL_BUYING_SET_sub* itemlist, int count, uint64 total_price)
{
	...
	pc_payzeny(sd, temp_price, LOG_TYPE_BUYING_STORE, sd->status.char_id);
	st->shop_type = 1;
	st->vender_id = stall_getuid();
	st->vend_num = i;
	st->expire_time = sd->stall_expire_time;
...
void stall_vending_purchasereq(map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
	...
	if (!searchstore_queryremote(*sd, st->vended_id) && (sd->m != st->m || !check_distance_bl(sd, st, AREA_SIZE)))
...
bool stall_searchall(map_session_data* sd, const struct s_search_store_search* s, const struct s_stall_data* st, short type)
{
	...
	if (!st->shop_type == type) // not good type
		return true;
